...
function acSequenceDetail(a){
 ...
 '<div class="ac-detail-section"><div class="ac-detail-title"><strong>Actividad reciente</strong></div><div class="ac-sequence-history">'+(history.length?history.map(function(h){const label=h.status==="completed"?"Completada":h.status==="partial"?"Parcial":"Omitida",icon=h.status==="completed"?"check":h.status==="partial"?"circle-dot-dashed":"skip-forward";return '<div><span class="'+h.status+'">'+ic(icon)+'</span><p><strong>'+acEsc(h.name)+'</strong><small>'+acDateLabel(h.date)+' · '+label+'</small></p></div>'}).join(""):'<div class="ac-history-empty">Todavía no hay sesiones registradas.</div>')+'</div></div>'
}
