...
function lpProgressionFieldLabel(f){return f==="weight"?"Carga ("+(typeof unitLabel==="function"?unitLabel():"kg")+")":f==="repsMax"?"Repeticiones":"Series"}
function lpOpenProgression(pi,phi){
 ...
 document.getElementById("lp-create-prog").onclick=function(){const key=document.getElementById("lp-prog-ex").value,e=exs.find(function(x){return x.key===key}),field=document.getElementById("lp-prog-field").value;let start=lpNum(document.getElementById("lp-prog-start").value,field==="weight"&&typeof toDisplayWeight==="function"?toDisplayWeight(e[field]):e[field]),end=lpNum(document.getElementById("lp-prog-end").value,start);if(field==="weight"&&typeof fromDisplayWeight==="function"){start=fromDisplayWeight(start);end=fromDisplayWeight(end)}const span=Math.max(1,ph.to-ph.from),values={};for(let w=ph.from;w<=ph.to;w++){let val=start+(end-start)*((w-ph.from)/span);val=field==="weight"?Math.round(val*2)/2:Math.round(val);values[String(w)]=val}plan.progressions=plan.progressions.filter(function(x){return !(x.exerciseKey===key&&x.field===field)});plan.progressions.push({id:"prog-"+Date.now(),exerciseKey:key,exerciseName:e.name,field:field,values:values});lpDirty(p);lpOpenPhase(pi,phi)}
}
